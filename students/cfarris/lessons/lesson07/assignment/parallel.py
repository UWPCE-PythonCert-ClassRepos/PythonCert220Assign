...
This file could use more error checking of the imported data.
pylint returns a rating of 9.56 after disabling three areas(see below)
These improvements will be tabled for future edits as they aren't essential
to the lesson 7 assignment requirements.
"""
...
def import_data(db, customers=None, products=None, rentals=None, start_time=0):
    """
    Adds files to collection. if the file/date
    type (k) fits into three categories
    ...
    threads = [threading.Thread(target=_read_csv,
                                args=(db,
                                      products_db,
                                      products,
                                      data_list,
                                      start_time)),
               threading.Thread(target=_read_csv,
                                args=(db,
                                      rentals_db,
                                      rentals,
                                      data_list,
...
def _read_csv(db, collection, csv_file, data_list=None, start_time=0):
    '''
    Opens csv files, creates a list of dictionarys
    with matching keys from header to values
    returns the dictionary and saves it to different
    mongodb collections based upon the
...
def main(start_time):
    '''
    runs methods within this script to add
    three csv files to their own collections
    ...
    with mongo:
        ...
        import_data(db, customers, products, rentals, start_time)
