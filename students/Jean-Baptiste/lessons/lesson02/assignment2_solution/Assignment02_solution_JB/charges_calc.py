...
def load_rentals_file(filename):
    with open(filename) as file:
        try:
            ...
            logging.error("There is an error loading json from file {}".format(filename))
...
def calculate_additional_fields(data):
    for value in data.values():
        try:
            ...
            logging.error("There is a calcul error input data, please try one more time")
            logging.exception("message")
            exit(1)
            value['unit_cost'] = value['total_price'] / value['units_rented']
    return data
...
def parse(self, inpath, outpath):
    ...
    with open(outpath, 'w') as fout:
        for l in g:
            ...
            clean_json=save_to_json
            clean_json.parse("source.json", "cleaned.json")
            clean_json.save()
