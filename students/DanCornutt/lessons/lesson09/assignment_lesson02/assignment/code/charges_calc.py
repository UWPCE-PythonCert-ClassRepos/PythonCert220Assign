...
def log_dec(func):
    """Decorator Function turns off debugger, runs function, then
    turns debugger back on
    param1: function to be decorated
    """
...
def load_rentals_file(filename):
    ...
    with open(filename) as file:
        try:
            ...
            logger.error("Error loading json. Exiting program.")
            exit(0)
    return data
    ...
    for value in data.values():
        ...
        try:
            ...
            if value['rental_end'] == "":
                ...
                logger.error('setting rental_end date to now')
            ...
            if value['units_rented'] < 1:
                ...
                logger.error("Math failure. Setting unit cost equal to total \
                    price.")
            else:
                value['unit_cost'] = (value['total_price'] /
                                      value['units_rented'])
        except ValueError:
            logger.error("Value error in data, continuing...")
            continue
