...
def add_customers_csv(fpath='customer.csv'):
    """Reads csv by line. Uses csv iterator to loop through csv and add
    customer to db"""
    ...
    for row in csv_f:
        if len(row) == len(headers):
            ...
            logging.error("""Error reading csv file for {row}.
            Program has skipped line.""".format(row))
    f.close()
